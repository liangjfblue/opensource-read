...
type Server struct {
	...
	lis      map[net.Listener]bool // 存储listen socket, 用于优雅退出
	conns    map[transport.ServerTransport]bool // 当前活跃客户端链接
	serve    bool
	drain    bool
...
func (s *Server) RegisterService(sd *ServiceDesc, ss interface{}) {
	if ss != nil {
		...
		// HandlerType必须实现rpc接口
		if !st.Implements(ht) {
			logger.Fatalf("grpc: Server.RegisterService found the handler of type %v that does not satisfy %v", st, ht)
		}
	}
	// 服务注册(其实就是通过代码生成的服务描述信息/反射得到的服务信息, 保存到map中[服务名映射到服务响应函数])
	s.register(sd, ss)
}
...
func (s *Server) Serve(lis net.Listener) error {
	...
	// 保存listen socket, GracefulStop先关闭, 再等待当前客户端链接完成, 最后退出
...
func (s *Server) processUnaryRPC(t transport.ServerTransport, stream *transport.Stream, info *serviceInfo, md *MethodDesc, trInfo *traceInfo) (err error) {
	...
	if err := s.sendResponse(t, stream, reply, cp, opts, comp); err != nil {
		// 服务端写完关闭链接, 服务端继续读, 报错EOF, 服务端知道客户端读完了
		if err == io.EOF {
...
func (s *Server) GracefulStop() {
	...
	// 等待server退出, serve() serveWG.Add(1)
	s.mu.Unlock()
	s.serveWG.Wait()
	s.mu.Lock()
